...
void main()
{
    ...
    if (fs_UV.x >= 14.f / 16.f && fs_UV.y >= 3.f / 16.f &&
       fs_UV.x <= 15.f / 16.f && fs_UV.y <= 4.f / 16.f) {
        float tx = modelposition.x * 0.1 + u_Time;
        float tz = modelposition.z * 0.1 + u_Time;
...
//    modelposition.y += (sin(modelposition.x * 0.1) + cos(modelposition.z * 0.1)) * 10;
    vec3 toPlayer = modelposition.xyz - u_PlayerPos;
//    modelposition.y *= ((cos(toPlayer.x * 3.14159 / 256.0) + 1) + cos(toPlayer.z * 3.14159 / 256.0) + 1) * 0.25;
