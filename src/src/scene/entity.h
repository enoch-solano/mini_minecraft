...
struct InputBundle {
    ...
    InputBundle(float currMouseX, float currMouseY)
        : wPressed(false), aPressed(false), sPressed(false),
          ...
          mouseX(currMouseX), mouseY(currMouseY),
          mouseXprev(currMouseX), mouseYprev(currMouseY)
    {}
};
...
public:
    ...
    virtual void moveAlongVector(glm::vec3 v);
